...
export default function DetailDataTable({ data, collapse = true }) {
    moment.locale("id");
        ...
        <table className="table w-full text-base table-bordered">
            ...
            <tbody>
                <RowData
                    ...
                    value={`${moment(data.created_at).format("LL")} (${moment(
                        data.created_at
                    ).fromNow()})`}
                />
                <tr>
                    <td>Diajukan Oleh</td>
                    <td>
                        <span className="block">{data.pengaju?.nama}</span>
                ...
                <tr>
                    ...
                    <td className="text-base font-normal">
                        {data["approved_pak_path"] ? (
                            ...
                            <span className="text-warning/80">Menunggu Validasi..</span>
                        )}
                    </td>
                </tr>
                <RowData
                    ...
                    value={
                        data.tanggal_ditolak
                            ? `${moment(data.tanggal_ditolak).format(
                                  "LL"
                              )} (${moment(data.tanggal_ditolak).fromNow()})`
                    ...
                    value={
                        data.tanggal_ditolak
                            ? `${moment(data.tanggal_direvisi).format(
                                  "LL"
                              )} (${moment(data.tanggal_direvisi).fromNow()})`
                    ...
                    value={
                        data.tanggal_divalidasi
                            ? `${moment(data.tanggal_divalidasi).format(
                                  "LL"
                              )} (${moment(data.tanggal_divalidasi).fromNow()})`
                            : "Belum Divalidasi"
                    }
                />
            </tbody>
