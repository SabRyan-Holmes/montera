            ...
            <main className="min-h-screen p-6 mx-auto space-y-6 bg-gray-50/50">
                ...
                <div className="flex flex-col items-center justify-between mb-6 md:flex-row">
                    <div>
                        ...
                        <p className="text-sm text-gray-500">
                            Ringkasan kinerja operasional & finansial cabang tahun {new Date().getFullYear()}
                        </p>
                    </div>
                ...
                <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
                    ...
                    <div className="flex items-center justify-between p-6 bg-white border-l-4 border-blue-500 shadow-sm rounded-xl">
                        <div>
                            <p className="text-sm font-medium text-gray-500 uppercase">Total Realisasi (YTD)</p>
                            <p className="mt-1 text-2xl font-bold text-gray-800">
                                {formatRupiah(quickStats.total_realisasi_tahun_ini)}
                            </p>
                        </div>
                    ...
                    <div className="flex items-center justify-between p-6 bg-white border-l-4 border-green-500 shadow-sm rounded-xl">
                        <div>
                            <p className="text-sm font-medium text-gray-500 uppercase">Nasabah Baru</p>
                            <p className="mt-1 text-2xl font-bold text-gray-800">
                                {quickStats.total_nasabah_baru} <span className="text-sm font-normal text-gray-400">Orang</span>
                            </p>
                        </div>
                    ...
                    <div className="flex items-center justify-between p-6 bg-white border-l-4 border-orange-500 shadow-sm rounded-xl">
                        <div>
                            <p className="text-sm font-medium text-gray-500 uppercase">Avg. per Transaksi</p>
                            <p className="mt-1 text-2xl font-bold text-gray-800">
                ...
                <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                    ...
                    <div className="p-6 bg-white border border-gray-100 shadow-sm lg:col-span-2 rounded-xl">
                        ...
                        <ResponsiveContainer width="100%" height={320}>
                            <BarChart data={trendData}>
                                ...
                                <XAxis dataKey="bulan" axisLine={false} tickLine={false} tick={{fill: '#6b7280'}} />
                                <YAxis axisLine={false} tickLine={false} tick={{fill: '#6b7280'}} tickFormatter={(val) => `${val/1000000}Jt`} />
                                <Tooltip
                                    cursor={{fill: '#f9fafb'}}
                    ...
                    <div className="p-6 bg-white border border-gray-100 shadow-sm rounded-xl">
                        <h2 className="mb-6 text-lg font-bold text-center text-gray-800">Komposisi Produk</h2>
                        <ResponsiveContainer width="100%" height={320}>
                            <PieChart>
                                <Pie
                                    data={chartData}
                    ...
                    <div className="p-6 bg-white border border-gray-100 shadow-sm rounded-xl">
                        ...
                        <div className="space-y-3">
                            {ranking.map((item, index) => (
                                <div key={index} className="flex items-center justify-between p-3 transition-colors border rounded-lg hover:bg-gray-50 border-gray-50">
                                    <div className="flex items-center gap-3">
                                        <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-white ${index === 0 ? 'bg-yellow-400' : index === 1 ? 'bg-gray-400' : index === 2 ? 'bg-orange-400' : 'bg-blue-100 text-blue-600'}`}>
                                            {index + 1}
                                        </div>
                                        <div>
                                            <p className="font-semibold text-gray-800">{item.pegawai?.name}</p>
                                            <p className="text-xs text-gray-500">{item.pegawai?.divisi?.nama_divisi || 'Tanpa Divisi'}</p>
                                        </div>
                                    </div>
                    ...
                    <div className="p-6 bg-white border border-gray-100 shadow-sm rounded-xl">
                        ...
                        <div className="space-y-4">
                            {divisiData.map((divisi, index) => {
                                ...
                                const percentage = quickStats.total_realisasi_tahun_ini > 0
                                    ? (divisi.total_realisasi / quickStats.total_realisasi_tahun_ini) * 100
                                    : 0;

                                return (
                                    <div key={index}>
                                        <div className="flex justify-between mb-1">
                                            <span className="text-sm font-medium text-gray-700">{divisi.nama_divisi}</span>
                                            <span className="text-sm font-bold text-gray-900">{formatRupiah(divisi.total_realisasi)}</span>
                                        </div>
                                        ...
                                        <div className="mt-1 text-right">
                                            <span className="text-xs text-gray-400">{percentage.toFixed(1)}% Kontribusi</span>
                                        </div>
                                    </div>
                                );
                            })}
