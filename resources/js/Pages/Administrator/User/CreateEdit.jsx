...
import { FaUserTie, FaUsersGear } from "react-icons/fa6"; // Icon User & Admin
...
optionList
    ...
    // 1. STATE DINAMIS (Sesuai field tabel users)
    const { data, setData, post, put, processing, errors } = useForm({
        name: user?.name || "",
        nip: user?.nip || "",
        email: user?.email || "",
        password: "", // Password selalu kosong di awal (keamanan)
    ...
    const submit = (e) => {
        e.preventDefault();
        if (isEdit) {
            // Kirim ke method update di controller
            put(route("admin.user.update", user.id));
        } else {
            // Kirim ke method store di controller
            post(route("admin.user.store"));
        }
    };

    // Helper untuk opsi Jabatan & Divisi (Mapping ID & Nama)
                    ...
                    <form onSubmit={submit}>
                        ...
                        <div className="flex justify-center w-full my-6">
                            <SuccessButton
                                type="submit"
                                disabled={processing}
                                className={`gap-2 px-8 py-3 text-base border text-white shadow-lg ${
                                    isEdit
                                        ? "bg-secondary/90 hover:bg-secondary border-secondary/80"
                                        : ""
                                }`}
                            >
                                {isEdit ? (
