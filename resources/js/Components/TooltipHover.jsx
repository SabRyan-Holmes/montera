...
export default function TooltipHover({ message, className }) {
    return (
        <div
            ...
            -translate-x-1/2 bg-disabled-color rounded shadow-lg opacity-0 pointer-events-none left-1/2 top-full group-hover:opacity-100`}
        >
            <small className={`font-semibold ${className}`}>{message}</small>{" "}
            {/* Segitiga bawah tooltip */}
            <div className="absolute w-2 h-2 rotate-45 -translate-x-1/2 bg-disabled-color -top-1 left-1/2"></div>
        </div>
    );
}
