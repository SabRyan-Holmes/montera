   ...
   public function up(): void
    {
        Schema::create('targets', function (Blueprint $table) {
            $table->id();
            $table->foreignId('user_id')->constrained('users')->onDelete('cascade')->comment('Pegawai ter target');
            $table->foreignId('supervisor_id')
                  ->constrained('users')
                  ->comment('Supervisor pemilik target ini');
            ...
            $table->timestamps();
            $table->index(['supervisor_id', 'user_id', 'tahun', 'bulan'], 'idx_dashboard_target_spv');
